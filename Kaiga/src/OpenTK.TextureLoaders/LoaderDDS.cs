...
namespace OpenTK.TextureLoaders
{
    /// <summary> 
    ...
    public static class ImageDDS
    {
        ...
        public static void LoadFromDisk( string filename, out int texturehandle, out TextureTarget dimension )
        {
            #region Prep data
            ...
            try // Exceptions will be thrown if any Problem occurs while working on the file. 
            {
                    ...
                    switch ( (eFOURCC) pfFourCC )
                {
                    ...
                    case eFOURCC.DXT5:
                        ...
                        _IsCompressed = true;
                        break;
                    case eFOURCC.A16B16G16R16F :
                        _PixelInternalFormat = PixelInternalFormat.Rgba16f;
                        _IsCompressed = false;
                        _BytesPerBlock = 8;
                        break;
                    default:
                ...
                for ( int Slices = 0 ; Slices < _Depth ; Slices++ )
                {
                    ...
                    for ( int Level = 0 ; Level < _MipMapCount ; Level++ ) // start at base image
                    {
                        ...
                        int SurfaceSizeInBytes;

                        if ( _IsCompressed )
                        {
                            BlocksPerRow = ( Width + 3 ) >> 2;
                            BlocksPerColumn = ( Height + 3 ) >> 2;
                            SurfaceBlockCount = BlocksPerRow * BlocksPerColumn; //   // DXTn stores Texels in 4x4 blocks, a Color block is 8 Bytes, an Alpha block is 8 Bytes for DXT3/5
                            SurfaceSizeInBytes = SurfaceBlockCount * _BytesPerBlock;
                        }
                        else
                        {
                            BlocksPerRow = Width;
                            BlocksPerColumn = Height;
                            SurfaceBlockCount = BlocksPerRow * BlocksPerColumn;
                            SurfaceSizeInBytes = SurfaceBlockCount * _BytesPerBlock;
                        }
                        ...
                        { // Note: there could be a potential problem with non-power-of-two cube maps
                            ...
                            switch ( dimension )
                            {
                                ...
                                case TextureTarget.TextureCubeMap:
                                    ...
                                    {
                                        GL.TexImage2D( TextureTarget.TextureCubeMapPositiveX + Slices,
                                            Level,
                                            _PixelInternalFormat,
                                            Width,
                                            Height,
                            ...
                            if ( GLError != ErrorCode.NoError || width == 0 || height == 0 || internalformat == 0 )
                            {
